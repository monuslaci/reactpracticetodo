...
export default function WorkCard({ item, icon, type }) {
    ...
    return (
        <article className="flex min-h-52 flex-col justify-between rounded-3xl border border-gray-100 bg-white p-5 text-left shadow-sm transition-shadow hover:shadow-md" onClick={() => navigate(isProject ? `/projectsDetails/${item.id}` : `/tasksDetails/${item.id}`)}>
